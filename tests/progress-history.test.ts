...
test("weekends are optional and do not count in progress reports or streak", () => {
  const weeklyDates = getProgressReportDates("weekly", new Date("2026-10-05T12:00:00-03:00")).map(dateKey);

  assert.deepEqual(weeklyDates, ["2026-10-01", "2026-10-02", "2026-10-05"]);
  assert.equal(isProgressTrackingDate(new Date("2026-10-03T12:00:00-03:00")), false);
  assert.equal(isProgressTrackingDate(new Date("2026-10-05T12:00:00-03:00")), true);
  assert.equal(
    calculateProgressStreak(
...
test("reports and streak only count dates from the new start date", () => {
  const dates = getProgressReportDates("weekly", new Date("2026-10-02T12:00:00-03:00")).map(dateKey);

  assert.deepEqual(dates, ["2026-10-01", "2026-10-02"]);
  assert.equal(
    calculateProgressStreak(
      ["2026-09-01", "2026-09-21", "2026-10-01"],
      new Date("2026-10-02T12:00:00-03:00")
    ),
    1
  );
  assert.equal(progressTrackingStartDate, "2026-10-01");
  assert.equal(progressResetVersion, "2026-10-01-clean-history-v1");
});
