...
const stackVisuals: StackVisual[] = [
  ...
  { src: "https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/oracle/oracle-original.svg", color: "#f80000", matches: ["oracle"] },
...
export default function StackIcon({ stack }: { stack: string }) {
  ...
  const imageStyle: CSSProperties | undefined = visual?.invert ? { filter: "invert(1)" } : undefined;
  ...
  return (
    <span className="dayStackIcon" style={style} title={stack} role="img" aria-label={stack}>
      {visual && !imageFailed
        ? <img src={visual.src} alt="" loading="lazy" decoding="async" style={imageStyle} onError={() => setImageFailed(true)} />
